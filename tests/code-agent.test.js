...
describe('CodeAgent', () => {
	...
	describe('Constructor', () => {
		...
		it('should accept custom tools and toolExecutor', () => {
			const tools = [{ name: 'lookup', description: 'Look up a value', input_schema: { type: 'object', properties: { key: { type: 'string' } }, required: ['key'] } }];
		...
		it('should throw when tools provided without toolExecutor', () => {
			const tools = [{ name: 'lookup', description: 'Look up', input_schema: { type: 'object', properties: {} } }];
	...
	describe('Custom Tools', () => {
		const CUSTOM_TOOLS = [{ name: 'db_query', description: 'Run a database query', input_schema: { type: 'object', properties: { sql: { type: 'string' } }, required: ['sql'] } }];

		it('should include custom tools in _buildToolDefinitions()', () => {
			const agent = makeAgent({ tools: CUSTOM_TOOLS, toolExecutor: async () => ({}) });
			const toolNames = agent._tools.map(t => t.name);
	...
	describe('importantFiles', () => {
		...
		it('should resolve absolute paths outside workingDirectory', async () => {
			const externalDir = await realpath(await mkdtemp(join(os.tmpdir(), 'ak-claude-ext-')));
